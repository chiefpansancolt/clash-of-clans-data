...
- Fix Lava hound images for levels
- Fix Siege Machine Resource Type showing Gold instead of Elixir.
- Fix Gem Calculator not calculator proper for lower than 7 day time.

## [0.15.0] - 2026-04-18

### Fix

- Add TH level for 17 and 18 to Eagle Artillery to show count as 0 since its merged to TH at 17.

## [0.14.0] - 2026-04-16

### Added

#### Clan Capital Forge Module

New `ClanCapitalForge` class accessible via `clanCapital().forge()` with full data for all four
Forge groupings:

- **Daily Forge** — Capital Gold obtained per Town Hall level (TH6–18), increasing by 50 per level
- **Auto Forge** — Resource conversion rates (home village TH7–18 + builder base BH8–10) with Gold
  Pass discounted prices; Dark Elixir option from TH13+
- **Forge** — Full-cost resource conversion rates (same TH/BH tiers, no Gold Pass discount)
- **Available Forges** — Manual forge slot tiers (1–4 slots) and the Town Hall level required to
  unlock each

New query methods:

- `dailyForge()` / `dailyForgeAtTownHall(th)`
- `craftingTime()` — 3-day crafting time shared by Auto Forge and Forge
- `availableForges()` / `availableForgesAtTownHall(th)` — returns slot count at a given TH level
- `autoForge()` / `autoForgeAtTownHall(th)` / `autoForgeAtBuilderHall(bh)`
- `forgeRates()` / `forgeAtTownHall(th)` / `forgeAtBuilderHall(bh)`

New TypeScript interfaces exported from the package: `ClanCapitalDailyForgeEntry`,
`ClanCapitalHomeForgeEntry`, `ClanCapitalBuilderForgeEntry`, `ClanCapitalAvailableForgeEntry`,
`ClanCapitalForgeRates`, `ClanCapitalForgeData`

## [0.13.0] - 2026-04-16

### Fix

- Fix image usage for Hog Raiders

## [0.12.0] - 2026-04-16

### Fix

- Update Clan Capital level counts for Troops to accurately count based on district hall levels.

## [0.11.0] - 2026-04-16

### Added

#### Clan Capital Traps — Full Implementation

Added all 5 Clan Capital traps with complete level data, images, tests, and sample scripts.

New `ClanCapitalTrap` and `ClanCapitalTrapLevel` TypeScript interfaces in `src/types/trap.ts`:

- `triggerRadius` — trap activation radius in tiles
- `damageRadius?` — optional blast radius (Mine, Mega Mine)
- `projectileCount?` — optional per-level projectile count (Spear Trap)
- `availablePerCapitalHall?` — optional `CapitalHallAvailability[]`
- `availablePerDistrict` — `DistrictAvailability[]` with `countPerDistrictHall` indexed by DH level

New `ClanCapitalTraps` query class accessible via `clanCapital().traps()`:

## [0.10.0] - 2026-04-16

### Fix

- Fixed dataIds on Traps for Builder Base as they were on wrong ones.
