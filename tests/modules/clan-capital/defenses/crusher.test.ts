...
describe('crusher() [Clan Capital]', () => {
  const d = clanCapital().defenses().crusher().first()!;

  it('has correct id', () => {
    expect(d.id).toBe('crusher');
  });

  it('has correct name', () => {
    expect(d.name).toBe('Crusher');
  });
  ...
  it('first level has correct stats', () => {
    ...
    expect(lv1.stats.normal.damagePerShot).toBe(525);
  ...
  it('last level has correct stats', () => {
    ...
    expect(lv5.stats.normal.damagePerShot).toBe(875);
  ...
  it('all levels have a valid image path', () => {
    for (const lvl of d.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/defenses\/crusher\/normal\/level-\d+\.png$/,
      );
    }
  });
  ...
  it('has 4 availablePerDistrict entries', () => {
    expect(d.availablePerDistrict).toHaveLength(4);
  });

  it('modes.normal has correct range and attackSpeed', () => {
    expect(d.modes.normal.range).toBe(1.7);
    expect(d.modes.normal.attackSpeed).toBe(3.5);
