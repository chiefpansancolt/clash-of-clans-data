...
describe('hogRider() [Clan Capital]', () => {
  const t = clanCapital().troops().hogRider().first()! as ClanCapitalTroop;

  it('has correct identity', () => {
    expect(t.id).toBe('hog-rider');
    expect(t.name).toBe('Hog Rider');
  ...
  it('has correct root stats', () => {
    ...
    expect(t.movementSpeed).toBe(20);
  ...
  it('level 5 has correct stats', () => {
    ...
    expect(l5.stats.normal.damagePerShot).toBe(190);
  });

  it('has valid icon image path', () => {
    expect(t.images.icon).toBe('images/clan-capital/troops/hog-rider/icon.png');
  });

  it('all levels reference the normal level-1 image', () => {
    for (const lvl of t.levels) {
      expect(lvl.images.normal).toBe('images/clan-capital/troops/hog-rider/normal/level-1.png');
    }
  });
});
