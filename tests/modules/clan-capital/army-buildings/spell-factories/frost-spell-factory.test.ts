...
describe('frostSpellFactory() [Clan Capital]', () => {
  ...
  it('has correct identity', () => {
    expect(sf.id).toBe('frost-spell-factory');
    expect(sf.name).toBe('Frost Spell Factory');
  ...
  it('spellUnlocked is Frost Spell', () => {
    expect(sf.spellUnlocked).toBe('Frost Spell');
  });
  ...
  it('level 1 has correct stats', () => {
    ...
    expect(l1.buildCost).toBe(15000);
    expect(l1.buildCostResource).toBe('Capital Gold');
    expect(l1.districtHallRequired).toBe(3);
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of sf.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/spell-factories\/frost-spell-factory\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is buildersWorkshop', () => {
    expect(sf.availablePerDistrict).toHaveLength(1);
    const d = sf.availablePerDistrict[0];
    expect(d.district).toBe('buildersWorkshop');
    expect(d.countPerDistrictHall).toEqual([0, 0, 1, 1, 1]);
  });
});
