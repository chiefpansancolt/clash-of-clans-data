...
describe('healSpellFactory() [Clan Capital]', () => {
  ...
  it('has correct identity', () => {
    expect(sf.id).toBe('heal-spell-factory');
    expect(sf.name).toBe('Heal Spell Factory');
  ...
  it('spellUnlocked is Healing Spell', () => {
    expect(sf.spellUnlocked).toBe('Healing Spell');
  });
  ...
  it('level 5 has correct stats', () => {
    ...
    expect(l5.buildCost).toBe(200000);
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of sf.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/spell-factories\/heal-spell-factory\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is wizardValley with [1,1,1,1,1]', () => {
    expect(sf.availablePerDistrict).toHaveLength(1);
    const d = sf.availablePerDistrict[0];
    expect(d.district).toBe('wizardValley');
