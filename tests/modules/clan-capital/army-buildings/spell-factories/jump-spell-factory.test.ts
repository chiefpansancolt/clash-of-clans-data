...
describe('jumpSpellFactory() [Clan Capital]', () => {
  ...
  it('has correct identity', () => {
    expect(sf.id).toBe('jump-spell-factory');
    expect(sf.name).toBe('Jump Spell Factory');
  ...
  it('spellUnlocked is Jump Spell', () => {
    expect(sf.spellUnlocked).toBe('Jump Spell');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of sf.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/spell-factories\/jump-spell-factory\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is wizardValley', () => {
