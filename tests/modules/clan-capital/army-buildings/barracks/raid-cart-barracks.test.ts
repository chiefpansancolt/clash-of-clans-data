...
describe('raidCartBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .raidCartBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('raid-cart-barracks');
    expect(b.name).toBe('Raid Cart Barracks');
  ...
  it('troopUnlocked is Raid Cart', () => {
    expect(b.troopUnlocked).toBe('Raid Cart');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/raid-cart-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is buildersWorkshop with [1,1,1,1,1]', () => {
