...
describe('flyingFortressYard() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .flyingFortressYard()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('flying-fortress-yard');
    expect(b.name).toBe('Flying Fortress Yard');
  ...
  it('troopUnlocked is Flying Fortress', () => {
    expect(b.troopUnlocked).toBe('Flying Fortress');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/flying-fortress-yard\/normal\/level-\d+\.png$/,
      );
    }
  });
