...
describe('minionBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .minionBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('minion-barracks');
    expect(b.name).toBe('Minion Barracks');
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/minion-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is barbarianCamp', () => {
