...
describe('superGiantBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .superGiantBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('super-giant-barracks');
    expect(b.name).toBe('Super Giant Barracks');
  ...
  it('troopUnlocked is Super Giant', () => {
    expect(b.troopUnlocked).toBe('Super Giant');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/super-giant-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
