...
describe('powerPekkaBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .powerPekkaBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('power-pekka-barracks');
    expect(b.name).toBe('Power P.E.K.K.A Barracks');
  ...
  it('troopUnlocked is Power P.E.K.K.A', () => {
    expect(b.troopUnlocked).toBe('Power P.E.K.K.A');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/power-pekka-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
