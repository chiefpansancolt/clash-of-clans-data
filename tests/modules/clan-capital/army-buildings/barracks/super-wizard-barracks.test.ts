...
describe('superWizardBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .superWizardBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('super-wizard-barracks');
    expect(b.name).toBe('Super Wizard Barracks');
  ...
  it('troopUnlocked is Super Wizard', () => {
    expect(b.troopUnlocked).toBe('Super Wizard');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/super-wizard-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
