...
describe('sneakyArcherBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .sneakyArcherBarracks()
    .first()! as ClanCapitalBarracksBuilding;

  it('has correct identity', () => {
    expect(b.id).toBe('sneaky-archer-barracks');
    expect(b.name).toBe('Sneaky Archer Barracks');
  ...
  it('troopUnlocked is Sneaky Archer', () => {
    expect(b.troopUnlocked).toBe('Sneaky Archer');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/sneaky-archer-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
  ...
  it('availablePerDistrict is barbarianCamp with [1,1,1,1,1]', () => {
    ...
    expect(d.district).toBe('barbarianCamp');
