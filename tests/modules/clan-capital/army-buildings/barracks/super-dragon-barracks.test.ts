...
describe('superDragonBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .superDragonBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('super-dragon-barracks');
    expect(b.name).toBe('Super Dragon Barracks');
  ...
  it('troopUnlocked is Super Dragon', () => {
    expect(b.troopUnlocked).toBe('Super Dragon');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/super-dragon-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });

  it('availablePerDistrict is dragonCliffs', () => {
