...
describe('battleRamBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .battleRamBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('battle-ram-barracks');
    expect(b.name).toBe('Battle Ram Barracks');
  ...
  it('troopUnlocked is Battle Ram', () => {
    expect(b.troopUnlocked).toBe('Battle Ram');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/battle-ram-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
