...
describe('rocketBalloonBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .rocketBalloonBarracks()
    .first()! as ClanCapitalBarracksBuilding;

  it('has correct identity', () => {
    expect(b.id).toBe('rocket-balloon-barracks');
    expect(b.name).toBe('Rocket Balloon Barracks');
  ...
  it('troopUnlocked is Rocket Balloon', () => {
    expect(b.troopUnlocked).toBe('Rocket Balloon');
  });
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/rocket-balloon-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
