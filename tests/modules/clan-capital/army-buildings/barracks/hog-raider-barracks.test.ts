...
describe('hogRaiderBarracks() [Clan Capital]', () => {
  const b = clanCapital()
    .armyBuildings()
    .barracks()
    .hogRaiderBarracks()
  ...
  it('has correct identity', () => {
    expect(b.id).toBe('hog-raider-barracks');
    expect(b.name).toBe('Hog Raider Barracks');
  ...
  it('all levels have a valid normal image path', () => {
    for (const lvl of b.levels) {
      expect(lvl.images.normal).toMatch(
        /^images\/clan-capital\/army-buildings\/barracks\/hog-raider-barracks\/normal\/level-\d+\.png$/,
      );
    }
  });
