...
describe('goblinOutpost() [Clan Capital]', () => {
  const h = clanCapital().other().houses().goblinOutpost().first()! as ClanCapitalHouse;

  it('has correct identity', () => {
    expect(h.id).toBe('goblin-outpost');
    expect(h.name).toBe('Goblin Outpost');
  ...
  it('has valid normal and ruin image paths', () => {
    ...
    expect(l1.images.normal).toMatch(
      /^images\/clan-capital\/other\/houses\/goblin-outpost\/normal\/level-1\.png$/,
    );
    expect(l1.images.ruin).toMatch(
      /^images\/clan-capital\/other\/houses\/goblin-outpost\/ruin\/level-1\.png$/,
    );
  });
  ...
  it('availablePerDistrict includes goblinMines', () => {
    ...
    expect(d.countPerDistrictHall).toEqual([9, 9, 9, 9]);
  });
});
