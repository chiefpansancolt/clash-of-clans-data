...
describe('woodenCabin() [Clan Capital]', () => {
  const h = clanCapital().other().houses().woodenCabin().first()! as ClanCapitalHouse;

  it('has correct identity', () => {
    expect(h.id).toBe('wooden-cabin');
    expect(h.name).toBe('Wooden Cabin');
  ...
  it('has valid normal and ruin image paths', () => {
    ...
    expect(l1.images.normal).toMatch(
      /^images\/clan-capital\/other\/houses\/wooden-cabin\/normal\/level-1\.png$/,
    );
    expect(l1.images.ruin).toMatch(
      /^images\/clan-capital\/other\/houses\/wooden-cabin\/ruin\/level-1\.png$/,
    );
  });
  ...
  it('availablePerDistrict includes balloonLagoon', () => {
    const d = h.availablePerDistrict.find((e) => e.district === 'balloonLagoon')!;
    expect(d).toBeDefined();
    expect(d.countPerDistrictHall).toEqual([4, 4, 4, 4, 5]);
  });
});
