...
describe('thatchedHut() [Clan Capital]', () => {
  const h = clanCapital().other().houses().thatchedHut().first()! as ClanCapitalHouse;

  it('has correct identity', () => {
    expect(h.id).toBe('thatched-hut');
    expect(h.name).toBe('Thatched Hut');
  ...
  it('has valid normal and ruin image paths', () => {
    ...
    expect(l1.images.normal).toMatch(
      /^images\/clan-capital\/other\/houses\/thatched-hut\/normal\/level-1\.png$/,
    );
    expect(l1.images.ruin).toMatch(
      /^images\/clan-capital\/other\/houses\/thatched-hut\/ruin\/level-1\.png$/,
    );
  });
  ...
  it('availablePerDistrict includes barbarianCamp', () => {
    const d = h.availablePerDistrict.find((e) => e.district === 'barbarianCamp')!;
    expect(d).toBeDefined();
    expect(d.countPerDistrictHall).toEqual([6, 7, 7, 9, 9]);
  });
});
