import { airBomb } from '@/modules/home/traps/air-bomb';

describe('airBomb()', () => {
  it('returns a HomeTrap object', () => {
    const result = airBomb();
  ...
  it('base is home, category is trap', () => {
    expect(airBomb().base).toBe('home');
    expect(airBomb().category).toBe('trap');
  });

  it('size is 1x1', () => {
    expect(airBomb().size).toBe('1x1');
  });

  it('has 13 levels', () => {
    expect(airBomb().levels).toHaveLength(13);
  });
