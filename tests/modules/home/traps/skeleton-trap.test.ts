import { skeletonTrap } from '@/modules/home/traps/skeleton-trap';

describe('skeletonTrap()', () => {
  it('returns a HomeTrap object', () => {
    const result = skeletonTrap();
  ...
  it('base is home, category is trap', () => {
    expect(skeletonTrap().base).toBe('home');
    expect(skeletonTrap().category).toBe('trap');
  });

  it('size is 1x1', () => {
    expect(skeletonTrap().size).toBe('1x1');
  });

  it('has 5 levels', () => {
    expect(skeletonTrap().levels).toHaveLength(5);
  });

  it('constants: triggerRadius 5, single, both (configurable), specialAbility Spawns Skeletons', () => {
    const st = skeletonTrap();
  ...
  it('level 1 stats', () => {
    const l1 = skeletonTrap().levels[0];
  ...
  it('level 5 stats', () => {
    const l5 = skeletonTrap().levels[4];
  ...
  it('spawnedUnits increases L1-4, stays 5 at L5', () => {
    const levels = skeletonTrap().levels;
  ...
  it('all levels use Gold', () => {
    skeletonTrap().levels.forEach((l) => expect(l.buildCostResource).toBe('Gold'));
  });

  it('each level has normal and air images', () => {
    skeletonTrap().levels.forEach((l, i) => {
  ...
  it('availablePerTownHall has 18 entries', () => {
    expect(skeletonTrap().availablePerTownHall).toHaveLength(18);
  });

  it('TH1-7 have 0, TH8 has 2, TH10 has 3, TH14 has 4', () => {
    const apt = skeletonTrap().availablePerTownHall;
