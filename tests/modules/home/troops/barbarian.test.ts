...
describe('troops()', () => {
  ...
  it('has 10 troops total', () => {
    expect(home().troops().count()).toBe(10);
  });

  it('byType("regular") returns 10 troops', () => {
    expect(home().troops().byType('regular').count()).toBe(10);
  });
  ...
  it('withSuperTroop() returns 8 troops', () => {
    expect(home().troops().withSuperTroop().count()).toBe(8);
  });

  it('byTownHall(1) returns 10 troops (all L1 troops use TH1 convention)', () => {
    expect(home().troops().byTownHall(1).count()).toBe(10);
  });
});
