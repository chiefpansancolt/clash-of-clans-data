...
describe('troops()', () => {
  ...
  it('has 19 troops total', () => {
    expect(home().troops().count()).toBe(19);
  });
  ...
  it('byTownHall(1) returns 19 troops (all L1 troops use TH1 convention)', () => {
    expect(home().troops().byTownHall(1).count()).toBe(19);
  });
});
