...
for (const e of war.maxWarBaseLoot()) {
  log(
    `TH${e.townHallLevel}: ${e.goldAndElixir.toLocaleString()} Gold/Elixir, ${e.darkElixir} Dark Elixir`,
  );
}
...
for (const e of war.maxWarBaseOre()) {
  log(
    `TH${e.townHallLevel}: ${e.shinyOre} Shiny, ${e.glowyOre} Glowy, ${e.starryOre ?? '-'} Starry`,
  );
}
...
for (const tier of war.warBonus()) {
  ...
  log(
    `${tier.label}: ${th18.victory.goldAndElixir.toLocaleString()} G/E + ${th18.victory.darkElixir} DE`,
  );
}
