...
log('--- All Levels ---');
log('Lvl  HPS  HPulse      HP  Upgrade Cost  Upgrade Time  PH  TH');
for (const lvl of p.levels) {
  ...
  log(
    `  ${String(lvl.level).padStart(2)}  ${String(lvl.healingPerSecond).padStart(3)}  ${String(lvl.healingPerPulse).padStart(6)}  ${String(lvl.hitpoints).padStart(6)}  ${cost.padStart(12)}  ${ut.padStart(12)}   ${lvl.petHouseLevelRequired}   ${lvl.townHallRequired}`,
  );
}
log('');
