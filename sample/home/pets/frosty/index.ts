...
log(`summonCooldown:   ${p.summonCooldown}s`);
log('');

log('--- All Levels ---');
log('Lvl  DPS    DPH  Frostmites/S  MaxFrostmites      HP  Upgrade Cost  Upgrade Time  PH  TH');
for (const lvl of p.levels) {
  ...
  log(
    `  ${String(lvl.level).padStart(2)}  ${String(lvl.damagePerSecond).padStart(3)}  ${String(lvl.damagePerHit).padStart(5)}  ${String(lvl.frostmitesPerSummon).padStart(12)}  ${String(lvl.maxFrostmites).padStart(13)}  ${String(lvl.hitpoints).padStart(6)}  ${cost.padStart(12)}  ${ut.padStart(12)}   ${lvl.petHouseLevelRequired}   ${lvl.townHallRequired}`,
  );
}
log('');
