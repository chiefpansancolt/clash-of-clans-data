...
log(`numberOfTargets:      ${p.numberOfTargets}`);
log(`chainDamageDecay:     ${p.chainDamageDecay}%`);
log('');

log('--- All Levels ---');
log('Lvl  DPS  DPH     HP  Upgrade Cost  Upgrade Time  PH  TH');
for (const lvl of p.levels) {
  ...
  log(
    `  ${String(lvl.level).padStart(2)}  ${String(lvl.damagePerSecond).padStart(3)}  ${String(lvl.damagePerHit).padStart(3)}  ${String(lvl.hitpoints).padStart(6)}  ${cost.padStart(12)}  ${ut.padStart(12)}   ${lvl.petHouseLevelRequired}   ${lvl.townHallRequired}`,
  );
}
log('');
