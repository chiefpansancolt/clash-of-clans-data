...
for (const lvl of p.levels) {
  ...
  log(
    `  ${String(lvl.level).padStart(2)}  ${String(lvl.damagePerSecond).padStart(3)}  ${String(lvl.damagePerHit).padStart(5)}  ${String(lvl.poisonMaxDps).padStart(9)}  ${String(lvl.poisonSpeedDecreasePercent + '%').padStart(9)}  ${String(lvl.poisonAttackRateDecreasePercent + '%').padStart(11)}  ${String(lvl.hitpoints).padStart(6)}  ${cost.padStart(12)}  ${ut.padStart(12)}   ${lvl.petHouseLevelRequired}   ${lvl.townHallRequired}`,
  );
}
log('');
