...
const es = home().resourceBuildings().elixirStorage().first()!;

log('=== elixirStorage() ===');
log(`id:       ${es.id}`);
log(`name:     ${es.name}`);
log(`base:     ${es.base}`);
log(`category: ${es.category}`);
log(`size:     ${es.size}`);
log(`levels:   ${es.levels.length}`);
log('');
...
log('--- Elixir Storage Available per Town Hall ---');
log('TH  Count');
es.availablePerTownHall.forEach((e) => {
