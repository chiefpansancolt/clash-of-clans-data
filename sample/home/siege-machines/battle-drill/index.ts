...
log(`range:                ${m.range} tile`);
log(`attackSpeed:          ${m.attackSpeed}s`);
log('');

log('--- All Levels ---');
log('Lvl  DPS  DPA     HP  Research Cost  Research Time  Lab  TH');
for (const lvl of m.levels) {
  ...
  log(
    `  ${String(lvl.level).padStart(2)}  ${String(lvl.damagePerSecond).padStart(3)}  ${String(lvl.damagePerHit).padStart(4)}  ${String(lvl.hitpoints).padStart(6)}  ${cost.padStart(13)}  ${rt.padStart(13)}   ${lvl.laboratoryRequired}   ${lvl.townHallRequired}`,
  );
}
log('');
