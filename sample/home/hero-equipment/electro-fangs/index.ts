...
log(
  `  ability: chainDamageDecay=${e.ability?.['chainDamageDecay']} chainDistance=${e.ability?.['chainDistance']}`,
);
...
for (const lvl of e.levels) {
  log(
    `  ${lvl.level}: Dmg:${lvl.stats['damagePerHit']} Targets:${lvl.stats['numberOfTargets']} ` +
