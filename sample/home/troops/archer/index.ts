...
log(`id:           ${a.id}`);
log(`name:         ${a.name}`);
log(`troopType:    ${a.troopType}`);
log(`targetType:   ${a.targetType}`);
log(`damageType:   ${a.damageType}`);
log(`housingSpace: ${a.housingSpace}`);
log(`movementSpeed:${a.movementSpeed}`);
log(`attackSpeed:  ${a.attackSpeed}s`);
log(`range:        ${a.range}`);
log(`barracks lvl: ${a.barrackLevelRequired}`);
log('');
...
log('--- Super Troop: Super Archer ---');
const st = a.superTroop!;
log(`id:           ${st.id}`);
...
let passed = 0;
let failed = 0;
if (checkImage('icon', a.images.icon)) passed++;
else failed++;
for (const lvl of a.levels) {
