...
log(`triggerHousingSpace:  ${gb.triggerHousingSpace}`);
log(`damageRadius:         ${gb.damageRadius} tiles`);
log(`damageType:           ${gb.damageType}`);
log(`targetType:           ${gb.targetType}`);
log(`specialAbility:       ${gb.specialAbility}`);
...
gb.levels.forEach((l) => {
  ...
  const bt =
    [
      ...
      .padStart(10) || '   instant';
  log(
    `  ${String(l.level).padStart(2)}   ${String(l.damage).padStart(6)} ${cost}  ${bt}  ${String(l.xpGained).padStart(6)}   ${l.townHallRequired}`,
  );
});
log('');
