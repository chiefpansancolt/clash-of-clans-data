...
const t = builder().troops().powerPekka().first()!;

log('=== powerPekka() [Builder Base] ===');
log(`id:                    ${t.id}`);
log(`name:                  ${t.name}`);
log(`base:                  ${t.base}`);
...
for (const lvl of t.levels) {
  ...
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | dpa: ${lvl.damagePerShot} | overcharge: ${lvl.overchargeDamage} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
