...
const t = builder().troops().babyDragon().first()!;

log('=== babyDragon() [Builder Base] ===');
log(`id:           ${t.id}`);
log(`name:         ${t.name}`);
log(`base:         ${t.base}`);
...
for (const lvl of t.levels) {
  ...
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | tantrum: +${lvl.tantrumDamageBonus}% | sneeze: ${lvl.fierySneezeDamageMin}-${lvl.fierySneezeDamageMax} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
