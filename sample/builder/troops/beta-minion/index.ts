...
const t = builder().troops().betaMinion().first()!;

log('=== betaMinion() [Builder Base] ===');
log(`id:           ${t.id}`);
log(`name:         ${t.name}`);
log(`base:         ${t.base}`);
...
for (const lvl of t.levels) {
  ...
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | uPC: ${lvl.unitsPerCamp} | psAtk: ${lvl.powerShotAttacks} | psDmg: ${lvl.powerShotDamagePerHit} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
