...
const t = builder().troops().nightWitch().first()!;

log('=== nightWitch() [Builder Base] ===');
log(`id:           ${t.id}`);
log(`name:         ${t.name}`);
log(`base:         ${t.base}`);
...
for (const lvl of t.levels) {
  ...
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | cooldown: ${lvl.batSummonCooldown}s | bats: ${lvl.batsPerSummon}(max ${lvl.batsMax}) | swarms: ${lvl.batSwarmCount} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
