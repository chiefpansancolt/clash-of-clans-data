...
const t = builder().troops().boxerGiant().first()!;

log('=== boxerGiant() [Builder Base] ===');
log(`id:           ${t.id}`);
log(`name:         ${t.name}`);
log(`base:         ${t.base}`);
...
log(`passiveAbility: ${t.passiveAbility}`);
log(`builderBarracksRequired: ${t.builderBarracksRequired}`);
log('');

log('--- Level Stats ---');
for (const lvl of t.levels) {
  const block = lvl.boxerBlockDurationSeconds ? `${lvl.boxerBlockDurationSeconds}s` : 'N/A';
  const research =
    lvl.researchCost === 0
      ? 'N/A'
      : `${lvl.researchCost.toLocaleString()} BE, ${formatBuildTime(lvl.researchTime)}`;
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | block: ${block} | pp: ${lvl.powerPunchDamage} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
