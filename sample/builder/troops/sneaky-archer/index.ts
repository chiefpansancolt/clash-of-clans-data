...
const t = builder().troops().sneakyArcher().first()!;

log('=== sneakyArcher() [Builder Base] ===');
log(`id:           ${t.id}`);
log(`name:         ${t.name}`);
log(`base:         ${t.base}`);
...
for (const lvl of t.levels) {
  const cloak = lvl.cloakDurationSeconds ? `${lvl.cloakDurationSeconds}s` : 'N/A';
  const research =
    lvl.researchCost === 0
      ? 'N/A'
      : `${lvl.researchCost.toLocaleString()} BE, ${formatBuildTime(lvl.researchTime)}`;
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | dps: ${lvl.dps} | uPC: ${lvl.unitsPerCamp} | cloak: ${cloak} | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
