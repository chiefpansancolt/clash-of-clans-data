...
const t = builder().troops().hogGlider().first()!;

log('=== hogGlider() [Builder Base] ===');
log(`id:                    ${t.id}`);
log(`name:                  ${t.name}`);
log(`base:                  ${t.base}`);
...
for (const lvl of t.levels) {
  ...
  log(
    `lv${lvl.level} | hp: ${lvl.hitpoints} | crashDmg: ${lvl.damagePerShot} | stun: ${lvl.stunDuration}s | SL${lvl.starLabRequired} | research: ${research}`,
  );
}
log('');
