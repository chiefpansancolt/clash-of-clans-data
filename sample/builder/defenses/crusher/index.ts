...
const cr = builder().defenses().crusher().first()!;

log('=== crusher() [Builder Base] ===');
log(`id:       ${cr.id}`);
log(`name:     ${cr.name}`);
log(`base:     ${cr.base}`);
log(`category: ${cr.category}`);
log(`size:     ${cr.size}`);
log(`target:   ${cr.targetType}`);
log('');

log('--- Mode ---');
log('normal:', cr.modes.normal);
log('');

log('--- Available Per Builder Hall ---');
for (const a of cr.availablePerBuilderHall) {
  log(`  BH${a.builderHallLevel}: ${a.count}`);
}
log('');

log('--- All Levels ---');
for (const lvl of cr.levels) {
  log(
