...
const fc = builder().defenses().firecrackers().first()!;

log('=== firecrackers() [Builder Base] ===');
log(`id:       ${fc.id}`);
log(`name:     ${fc.name}`);
log(`base:     ${fc.base}`);
log(`category: ${fc.category}`);
log(`size:     ${fc.size}`);
log(`target:   ${fc.targetType}`);
log('');

log('--- Mode ---');
log('normal:', fc.modes.normal);
log('');

log('--- Available Per Builder Hall ---');
for (const a of fc.availablePerBuilderHall) {
