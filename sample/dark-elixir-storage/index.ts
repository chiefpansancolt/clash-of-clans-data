...
const ds = home().resourceBuildings().darkElixirStorage().first()!;

log('=== darkElixirStorage() ===');
log(`id:       ${ds.id}`);
log(`name:     ${ds.name}`);
log(`base:     ${ds.base}`);
log(`category: ${ds.category}`);
log(`size:     ${ds.size}`);
log(`levels:   ${ds.levels.length}`);
log('');
...
log('--- Dark Elixir Storage Available per Town Hall ---');
log('TH  Count');
ds.availablePerTownHall.forEach((e) => {
