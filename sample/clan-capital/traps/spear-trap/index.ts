...
log('--- Spear Trap Levels ---');
log('Lv  Damage  Spears  Build Cost  CH  DH');
for (const lvl of t.levels) {
  const cost = lvl.buildCost.toLocaleString();
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.damage).padStart(6)}  ${String(lvl.projectileCount ?? '').padStart(6)}  ${cost.padStart(10)}  ${String(lvl.capitalHallRequired).padStart(2)}   ${lvl.districtHallRequired}`,
  );
}
log('');
