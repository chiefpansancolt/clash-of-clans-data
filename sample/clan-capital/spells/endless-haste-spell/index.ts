...
const spells = clanCapital().spells().endlessHasteSpell();

log('=== endlessHasteSpell() [Clan Capital] ===');
...
for (const s of spells.get() as ClanCapitalSpell[]) {
  ...
  for (const lvl of s.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.speedIncrease).padStart(6)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
