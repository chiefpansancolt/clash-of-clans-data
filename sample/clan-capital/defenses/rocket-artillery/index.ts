...
log('=== rocketArtillery() [Clan Capital] ===');
log(`id:                          ${d.id}`);
log(`name:                        ${d.name}`);
log(`base:                        ${d.base}`);
...
log(`size:                        ${d.size}`);
log(`range:                       ${d.modes.normal.minRange}-${d.modes.normal.range} tiles`);
log(`attackSpeed:                 ${d.modes.normal.attackSpeed}s`);
log(`damageType:                  ${d.modes.normal.damageType}`);
log('');

log('--- Rocket Artillery Levels ---');
log('Lv  DPS    DPH    HP     Build Cost  CH  DH');
for (const lvl of d.levels) {
  const cost = lvl.buildCost.toLocaleString();
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.stats.normal.dps).padStart(3)}    ${String(lvl.stats.normal.damagePerShot).padStart(5)}  ${String(lvl.hitpoints).padStart(5)}  ${cost.padStart(10)}   ${lvl.capitalHallRequired}   ${lvl.districtHallRequired}`,
  );
}
log('');
