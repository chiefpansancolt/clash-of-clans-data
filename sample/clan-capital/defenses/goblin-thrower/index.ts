...
log('=== goblinThrower() [Clan Capital] ===');
log(`id:                          ${d.id}`);
log(`name:                        ${d.name}`);
log(`base:                        ${d.base}`);
...
log(`numberOfRounds:              ${d.modes.normal.numberOfRounds}`);
log('');

log('--- Goblin Thrower Levels ---');
log('Lv  DPS    DPH      HP     Build Cost  DH  TroopLv');
for (const lvl of d.levels) {
  const cost = lvl.buildCost.toLocaleString();
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.stats.normal.dps).padStart(3)}    ${String(lvl.stats.normal.damagePerShot).padStart(6)}  ${String(lvl.hitpoints).padStart(5)}  ${cost.padStart(10)}   ${lvl.districtHallRequired}       ${lvl.troopLevel}`,
  );
}
log('');
