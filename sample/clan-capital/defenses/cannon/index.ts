...
const d = clanCapital().defenses().cannon().first()!;

log('=== cannon() [Clan Capital] ===');
log(`id:                          ${d.id}`);
log(`name:                        ${d.name}`);
log(`base:                        ${d.base}`);
...
for (const lvl of d.levels) {
  ...
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.stats.normal.dps).padStart(3)}    ${String(lvl.stats.normal.damagePerShot).padStart(3)}    ${String(lvl.hitpoints).padStart(5)}  ${cost.padStart(10)}   ${lvl.capitalHallRequired}   ${lvl.districtHallRequired}`,
  );
}
log('');
