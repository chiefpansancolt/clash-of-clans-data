...
log('--- Bomb Tower Levels ---');
log('Lv  DPS    DPH     Death   HP     Build Cost  CH  DH');
for (const lvl of d.levels) {
  const cost = lvl.buildCost.toLocaleString();
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.stats.normal.dps).padStart(3)}    ${String(lvl.stats.normal.damagePerShot).padStart(5)}  ${String(lvl.deathDamage).padStart(5)}  ${String(lvl.hitpoints).padStart(5)}  ${cost.padStart(10)}   ${lvl.capitalHallRequired}   ${lvl.districtHallRequired}`,
  );
}
log('');
