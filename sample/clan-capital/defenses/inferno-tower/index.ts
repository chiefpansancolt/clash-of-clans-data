...
log('--- Inferno Tower Levels ---');
log('Lv  DPS(init)  DPS(2.5s)  DPS(7.5s)  HP     Build Cost  CH  DH');
for (const lvl of d.levels) {
  const cost = lvl.buildCost.toLocaleString();
  log(
    `${String(lvl.level).padStart(2)}  ${String(lvl.stats.normal.dps).padStart(8)}  ${String(lvl.stats.normal.dpsAfter2p5s).padStart(9)}  ${String(lvl.stats.normal.dpsAfter7p5s).padStart(9)}  ${String(lvl.hitpoints).padStart(5)}  ${cost.padStart(10)}   ${lvl.capitalHallRequired}   ${lvl.districtHallRequired}`,
  );
}
log('');
