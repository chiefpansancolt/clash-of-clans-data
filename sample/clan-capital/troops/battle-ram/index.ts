...
const troops = clanCapital().troops().battleRam();

log('=== battleRam() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  log(`wallDamageMultiplier:  ${t.wallDamageMultiplier}x`);
  log(`spawnedBarbarians:     ${t.spawnedBarbarians}`);
  log(`levels:                ${t.levels.length}`);
  images.check(`${t.id} icon`, t.images.icon);
  log('');

  log('Lv   HP   Damage  DmgVsWalls  DH Req');
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}   ${String(lvl.stats.normal.damagePerShot).padStart(6)}  ${String(lvl.stats.normal.damageVsWalls).padStart(10)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
