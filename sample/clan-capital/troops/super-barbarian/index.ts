...
const troops = clanCapital().troops().superBarbarian();

log('=== superBarbarian() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  log(`rageMovementSpeedIncrease: ${t.rageMovementSpeedIncrease}`);
  ...
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}  ${String(lvl.stats.normal.dps).padStart(4)}  ${String(lvl.stats.normal.damagePerShot).padStart(10)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
