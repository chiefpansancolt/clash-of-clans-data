...
const troops = clanCapital().troops().hogRider();

log('=== hogRider() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}  ${String(lvl.stats.normal.dps).padStart(3)}  ${String(lvl.stats.normal.damagePerShot).padStart(10)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
