...
const troops = clanCapital().troops().skeletonBarrels();

log('=== skeletonBarrels() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  log('Lv   HP   DmgPerShot  Skeletons  DH Req');
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}  ${String(lvl.stats.normal.damagePerShot).padStart(10)}  ${String(lvl.spawnedSkeletons).padStart(9)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
