...
const troops = clanCapital().troops().superWizard();

log('=== superWizard() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  log('Lv   HP  DPS  DmgPerShot  ChainDmg  DH Req');
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}  ${String(lvl.stats.normal.dps).padStart(3)}  ${String(lvl.stats.normal.damagePerShot).padStart(10)}  ${String(lvl.stats.normal.chainDamagePerShot).padStart(8)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
