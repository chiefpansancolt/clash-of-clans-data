...
const troops = clanCapital().troops().mountainGolem();

log('=== mountainGolem() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(5)}  ${String(lvl.stats.normal.dps).padStart(4)}  ${String(lvl.stats.normal.damagePerShot).padStart(10)}  ${String(lvl.deathDamage).padStart(8)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
