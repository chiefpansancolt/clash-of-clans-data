...
const troops = clanCapital().troops().infernoDragon();

log('=== infernoDragon() [Clan Capital] ===');
...
for (const t of troops.get() as ClanCapitalTroop[]) {
  ...
  log('Lv    HP  Init DPS  After1.7s  After3.2s  DH Req');
  for (const lvl of t.levels) {
    log(
      `${String(lvl.level).padStart(2)}  ${String(lvl.hitpoints).padStart(4)}  ${String(lvl.stats.normal.infernoInitialDps).padStart(8)}  ${String(lvl.stats.normal.infernoDpsAfter1_7s).padStart(9)}  ${String(lvl.stats.normal.infernoDpsAfter3_2s).padStart(9)}  ${String(lvl.districtHallRequired).padStart(6)}`,
    );
